...
/**
 * This class provides test cases for the Wall class.
 */
class WallsTest {
    Walls wall;
    Grid grid;
    ...
    void calculateUpNormal() {
        assertThat(wall.calculateUpNormal()).isEqualTo(
                new double[]{2.0523359562429437, 2.0013704652454263});
    }

    @Test
    void calculateDownNormal() {
        assertThat(wall.calculateDownNormal()).isEqualTo(
                new double[]{1.9476640437570563, 3.9986295347545737});
    }

    @Test
    void calculateRightNormal() {
        assertThat(wall.calculateRightNormal()).isEqualTo(
                new double[]{1.0013704652454263, 2.9476640437570563});

    }

    @Test
    void calculateLeftNormal() {
        assertThat(wall.calculateLeftNormal()).isEqualTo(
                new double[]{2.9986295347545737, 3.0523359562429437});
    }

    @Test
    void topDistancesToBall() {

        assertThat(wall.topDistancesToBall(ball)).isEqualTo(
                new double[]{41.45919958770641, 58.975878943834445, 51.66451192243433});

    }

    @Test
    void bottomDistancesToBall() {
        assertThat(wall.bottomDistancesToBall(ball)).isEqualTo(
                new double[]{49.62213114215846, 65.23095019701454, 58.743866459282444});
